...
static rt_thread_t  gt915l_thread = RT_NULL;
static rt_sem_t     gt915l_sem = RT_NULL;
static rt_device_t  dev = RT_NULL;
static struct       rt_touch_data *read_data;
static struct       rt_touch_info info;

static void gt915l_entry(void *parameter)
{
    ...
    while (1)
    {
        rt_sem_take(gt915l_sem, RT_WAITING_FOREVER);
...
{
    rt_sem_release(gt915l_sem);
    rt_device_control(dev, RT_TOUCH_CTRL_DISABLE_INT, RT_NULL);
    return 0;
}

/* Test function */
int gt915l_sample(const char *name, rt_uint16_t x, rt_uint16_t y)
{
    void *id;
    ...
    gt915l_sem = rt_sem_create("dsem", 0, RT_IPC_FLAG_FIFO);
    if (gt915l_sem == RT_NULL)
    {
    ...
    gt915l_thread = rt_thread_create("thread1",
                                     gt915l_entry,
                                     RT_NULL,
                                     THREAD_STACK_SIZE,
                                     THREAD_PRIORITY,
    ...
    if (gt915l_thread != RT_NULL)
        rt_thread_startup(gt915l_thread);
